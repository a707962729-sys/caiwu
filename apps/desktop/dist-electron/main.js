...
const __dirname$1 = require$$1$1.dirname(require$$2$1.fileURLToPath(typeof document === "undefined" ? require("url").pathToFileURL(__filename).href : _documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === "SCRIPT" && _documentCurrentScript.src || new URL("main.js", document.baseURI).href));
let mainWindow = null;
let apiServer = null;
...
function isApiServerRunning() {
  return new Promise((resolve) => {
    ...
    const req = http.get(`http://localhost:${API_PORT}/api/health`, { timeout: 2e3 }, (res) => {
...
async function startApiServer() {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      const dbPath = isDev ? require$$1$1.join(__dirname$1, "../../../data/caiwu.db") : require$$1$1.join(require$$1$3.app.getPath("userData"), "data/caiwu.db");
      const fs2 = require("fs");
      const dataDir = require$$1$1.dirname(dbPath);
      ...
      const apiPath = isDev ? require$$1$1.join(__dirname$1, "../../../services/api/src/index.js") : require$$1$1.join(process.resourcesPath, "api/src/index.js");
...
function createWindow() {
  mainWindow = new require$$1$3.BrowserWindow({
    ...
    icon: require$$1$1.join(__dirname$1, "../public/icon.png"),
    show: false
    // 先隐藏，加载完成后再显示
  });
...
require$$1$3.app.whenReady().then(async () => {
  ...
  if (shouldStartApi) {
    try {
      ...
      require$$1$3.dialog.showErrorBox("启动失败", "API 服务启动失败，应用可能无法正常工作。");
    }
  }
  createWindow();
...
require$$1$3.app.on("before-quit", async (e) => {
  console.log("[Caiwu Desktop] App quitting...");
  e.preventDefault();
  await stopApiServer();
  require$$1$3.app.exit(0);
});
...
require$$1$3.ipcMain.on("install-update", () => {
  main$1.autoUpdater.quitAndInstall();
});
require$$1$3.ipcMain.on("check-update", async () => {
  try {
    await main$1.autoUpdater.checkForUpdates();
  } catch (error2) {
    console.error("Update check failed:", error2);
  }
});
